function m_I2V2C(image,tablename)
...
in = inpolygon(x3, y3, X2, Y2);
   if (in ==1)
   subquestion(x2,y2,id,t,x3,y3,tablename);
   end
end 
...
function subquestion(X,Y,id,t,x1,y1,name)
...
fid1=fopen('result.txt','at+');       
fprintf(fid1, '%s(id,x,y,t)  :- id=%d,t=%d',name,id,t);
    for p=1:3  
            if vx(1,p)==vx(2,p) 
